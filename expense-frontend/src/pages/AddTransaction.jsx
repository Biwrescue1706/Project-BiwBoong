...
import { getAccountTypes } from "../services/accountType.service";
...
function AddTransaction() {
  ...
  const loadAccountTypes = async () => {
    try {
      ...
      console.error(err);
      setAccountTypes([]);
      errorAlert(err.response?.data?.message || "ไม่สามารถโหลดช่องทางบัญชีได้");
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.accountTypesId) return errorAlert("กรุณาเลือกช่องทางบัญชี");
    ...
    try {
      ...
      await createTransaction({
        ...
        accountTypesId: form.accountTypesId,
  ...
  const getTypeStyle = (item) => {
    ...
    return {
      container: "border-gray-200 bg-white hover:border-green-300 hover:bg-gray-50",
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="space-y-6 p-5 sm:p-6 md:p-7">
                  ...
                  {types.map((item) => {
                    ...
                    return (
                      <button
                        ...
                        className={`group flex min-h-[76px] items-center justify-center gap-3 rounded-2xl border-2 px-4 py-3 transition-all duration-200 active:scale-[0.98] ${styles.container}`}
                      >
                        <div
                          className={`flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl transition-transform duration-200 group-hover:scale-105 ${styles.icon}`}
                        >
                          {income ? (
                            <FaArrowUp />
                          ) : expense ? (
                        ...
                        <span
                          className={`text-sm font-bold sm:text-base ${styles.text}`}
                        >
                          {item.name}
                        </span>
                      </button>
              ...
              <div>
                ...
                <div className="relative">
                  <select
                    ...
                    className="h-13 w-full appearance-none rounded-xl border border-gray-200 bg-gray-50 px-4 py-3.5 pr-10 text-sm font-medium text-black outline-none transition focus:border-green-500 focus:bg-white focus:ring-4 focus:ring-green-500/10 disabled:cursor-not-allowed disabled:bg-gray-100 disabled:text-black"
                  >
                    <option value="">
                      {form.typeId
                        ? "เลือกหมวดหมู่"
              ...
              <div>
                ...
                <div className="relative">
                  <select
                    name="accountTypesId"
                    value={form.accountTypesId}
                    onChange={handleChange}
                    className="h-13 w-full appearance-none rounded-xl border border-gray-200 bg-gray-50 px-4 py-3.5 pr-10 text-sm font-medium text-black outline-none transition focus:border-green-500 focus:bg-white focus:ring-4 focus:ring-green-500/10"
                  >
                    <option value="">เลือกช่องทางบัญชี</option>
              ...
              <div>
                ...
                <div className="relative">
                  <div
                    className={`pointer-events-none absolute left-4 top-1/2 flex h-9 w-9 -translate-y-1/2 items-center justify-center rounded-lg ${isIncome
                  ...
                  <input
                    ...
                    step="0.01"
                    placeholder="0.00"
                    className={`h-16 w-full rounded-2xl border bg-white pl-16 pr-16 text-xl font-extrabold text-black outline-none transition placeholder:text-gray-400 ${isIncome
                ...
                {form.amount && (
                  <p
                    className={`mt-2 text-xs font-semibold ${isIncome
                      ? "text-green-600"
                      : isExpense
                        ? "text-red-600"
                        : "text-black"
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    value={form.date}
                    onChange={handleChange}
                    className="h-13 w-full rounded-xl border border-gray-200 bg-gray-50 px-11 py-3.5 text-sm font-medium text-black outline-none transition focus:border-green-500 focus:bg-white focus:ring-4 focus:ring-green-500/10"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full resize-none rounded-2xl border border-gray-200 bg-gray-50 px-4 py-3.5 text-sm font-medium text-black outline-none transition placeholder:text-gray-500 focus:border-green-500 focus:bg-white focus:ring-4 focus:ring-green-500/10"
                />
            ...
            <div className="border-t border-gray-100 bg-gray-50 p-5 sm:p-6">
              <button
                type="submit"
                disabled={loading}
                className="group flex min-h-[52px] w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-green-600 to-emerald-600 px-5 py-3.5 text-sm font-bold text-white shadow-lg shadow-green-500/20 transition-all duration-200 hover:-translate-y-0.5 hover:from-green-700 hover:to-emerald-700 hover:shadow-xl active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-60"
              >
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-xs leading-5 text-black sm:text-sm">
                เลือกประเภท หมวดหมู่ และช่องทางบัญชีให้ถูกต้อง
                เพื่อให้ยอดคงเหลือของแต่ละบัญชีถูกต้อง
              </p>
            </div>
          </div>
        </div>
